 ...
 * @return void
 */
function cb_identityjs2026_enqueue_vendor( $handle, $file, $is_css = false ) {
	$rel = '/js/vendor/' . $file;
	$abs = get_stylesheet_directory() . $rel;
	if ( ! file_exists( $abs ) ) {
		return;
...
function cb_identityjs2026_enqueue_scripts() {
	...
	$abs = get_stylesheet_directory() . $rel;
	if ( file_exists( $abs ) ) {
		wp_enqueue_script( 'cb-identityjs2026-theme', get_stylesheet_directory_uri() . $rel, array(), filemtime( $abs ), true );
	}
}
