 ...
 * @return void
 */
function cb_identityjs2026_register_blocks() {
	foreach ( glob( get_template_directory() . '/blocks/*/block.json' ) as $block_json ) {
		register_block_type( dirname( $block_json ) );
	}
}
add_action( 'init', 'cb_identityjs2026_register_blocks' );
 ...
 * @return array
 */
function cb_identityjs2026_core_block_type_args( $args, $name ) {
	$GLOBALS['cb_identityjs2026_original_block_categories'][ $name ] = isset( $args['category'] ) ? $args['category'] : '';
	...
	if ( in_array( $name, $wrapped_blocks, true ) ) {
		$args['render_callback'] = 'cb_identityjs2026_wrap_block_in_container';
	}

	return $args;
}
add_filter( 'register_block_type_args', 'cb_identityjs2026_core_block_type_args', 10, 2 );
 ...
 * @return array
 */
function cb_identityjs2026_register_gutenberg_block_category( $categories ) {
...
add_filter( 'block_categories_all', 'cb_identityjs2026_register_gutenberg_block_category' );
 ...
 * cb_identityjs2026_get_disallowed_gutenberg_blocks()), which silently hides
 * every block registered under a colliding "theme" slug too.
 *
 * @param array $categories Existing block categories.
 * @return array
 */
function cb_identityjs2026_register_theme_block_category( $categories ) {
	$theme = wp_get_theme();
...
add_filter( 'block_categories_all', 'cb_identityjs2026_register_theme_block_category' );
 ...
 */
function cb_identityjs2026_get_disallowed_gutenberg_blocks() {
	return array(
		'categories'        => array(
			'theme',
			'embed',
 ...
 * @return bool|array
 */
function cb_identityjs2026_disallow_unwanted_core_blocks( $allowed_block_types ) {
	$registered_blocks       = WP_Block_Type_Registry::get_instance()->get_all_registered();
	$disallowed_gutenberg    = cb_identityjs2026_get_disallowed_gutenberg_blocks();
	$disallowed_blocks = array_keys(
		array_filter(
			$registered_blocks,
			static function ( $block_type, $block_name ) use ( $disallowed_gutenberg ) {
				$original_categories    = isset( $GLOBALS['cb_identityjs2026_original_block_categories'] ) ? $GLOBALS['cb_identityjs2026_original_block_categories'] : array();
...
add_filter( 'allowed_block_types_all', 'cb_identityjs2026_disallow_unwanted_core_blocks' );
 ...
 * @return string
 */
function cb_identityjs2026_wrap_block_in_container( $attributes, $content ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found
