 ...
 * @return void
 */
function cb_identityjs2026_preload_fonts() {
...
add_action( 'wp_head', 'cb_identityjs2026_preload_fonts', 1 );
 ...
 * @return void
 */
function cb_identityjs2026_head_tags() {
	if ( ! is_user_logged_in() ) {
		$ga_property = cb_identityjs2026_get_setting( 'ga_property' );
		if ( $ga_property ) {
			?>
			<!-- Google Analytics -->
		...
		$gtm_property = cb_identityjs2026_get_setting( 'gtm_property' );
		if ( $gtm_property ) {
			?>
			<!-- Google Tag Manager -->
			<script>
	...
	$google_verification = cb_identityjs2026_get_setting( 'google_site_verification' );
	if ( $google_verification ) {
		printf( '<meta name="google-site-verification" content="%s" />' . "\n", esc_attr( $google_verification ) );
	}

	$bing_verification = cb_identityjs2026_get_setting( 'bing_site_verification' );
	if ( $bing_verification ) {
		printf( '<meta name="msvalidate.01" content="%s" />' . "\n", esc_attr( $bing_verification ) );
	}
}
add_action( 'wp_head', 'cb_identityjs2026_head_tags', 1 );
 ...
 * @return void
 */
function cb_identityjs2026_gtm_noscript() {
	if ( is_user_logged_in() ) {
		return;
	}
...
add_action( 'wp_body_open', 'cb_identityjs2026_gtm_noscript' );
 ...
 * @return void
 */
function cb_identityjs2026_print_custom_scripts( $key ) {
	// Default-on, matching GA/GTM — see the checkbox renderer for why '0' and
	// not '' is the unticked value.
	if ( cb_identityjs2026_get_setting( 'custom_scripts_logged_out_only', '1' ) && is_user_logged_in() ) {
		return;
	}

	$markup = cb_identityjs2026_get_setting( $key );
	if ( ! $markup ) {
		return;
	}
 ...
 * @return void
 */
function cb_identityjs2026_custom_head_scripts() {
	cb_identityjs2026_print_custom_scripts( 'custom_head' );
}
add_action( 'wp_head', 'cb_identityjs2026_custom_head_scripts', 99 );
 ...
 * @return void
 */
function cb_identityjs2026_custom_body_open_scripts() {
	cb_identityjs2026_print_custom_scripts( 'custom_body_open' );
}
add_action( 'wp_body_open', 'cb_identityjs2026_custom_body_open_scripts', 20 );
 ...
 * @return void
 */
function cb_identityjs2026_custom_body_close_scripts() {
	cb_identityjs2026_print_custom_scripts( 'custom_body_close' );
}
add_action( 'wp_footer', 'cb_identityjs2026_custom_body_close_scripts', 99 );
