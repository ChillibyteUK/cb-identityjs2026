...
for i in "${!field_names[@]}"; do
  ...
  help_para=""
  [ -n "$help" ] && help_para="\n\t\t\t\t<p className=\"cb-identityjs2026-editor-field__help\">{ __( '${help}', '${theme_slug}' ) }</p>"
  ...
  case "$type" in
    ...
    textarea)
      ...
      ;;
    richtext)
      field_html+=("\t\t\t<div className=\"cb-identityjs2026-editor-field\">\n\t\t\t\t<label className=\"cb-identityjs2026-editor-field__label\">{ __( '${label}', '${theme_slug}' ) }</label>\n\t\t\t\t<RichText\n\t\t\t\t\ttagName=\"div\"\n\t\t\t\t\tclassName=\"cb-identityjs2026-editor-field__control\"\n\t\t\t\t\taria-label={ __( '${label}', '${theme_slug}' ) }\n\t\t\t\t\tplaceholder={ __( '${label}', '${theme_slug}' ) }\n\t\t\t\t\tvalue={ ${name} }\n\t\t\t\t\tonChange={ ( value ) => setAttributes( { ${name}: value } ) }\n\t\t\t\t/>${help_para}\n\t\t\t</div>\n")
      ;;
    checkbox)
...

{
  ...
  echo "	const blockProps = useBlockProps( { className: 'container cb-identityjs2026-editor-block' } );"
  echo ""
  echo "	return ("
